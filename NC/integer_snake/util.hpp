...
namespace Utils
{
    ...
    template <typename T>
    void index_after_sorting(const std::vector<T>& qualities, std::vector<int>& sorted, int max_index=-1);
...
template <typename T>
void Utils::index_after_sorting(const std::vector<T>& qualities, std::vector<int>& sorted, int max_index)
{
    sorted.resize(qualities.size());
    auto begin = sorted.rbegin();
    for(int i=0; i<(int)qualities.size()-max_index && begin != sorted.rend(); i++, begin++)
        ;
    std::iota(begin, sorted.rend(), 0);
    std::sort(begin, sorted.rend(), [&](int a, int b){return qualities[a] > qualities[b];});
}
