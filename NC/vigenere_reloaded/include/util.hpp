...
namespace Utils
{
    ...
    void index_after_sorting(const std::vector<T>& qualities, std::vector<int>& sorted, int begin_at = 0);
...
template <typename T, typename U>
void Utils::index_after_sorting(const std::vector<T>& qualities, std::vector<int>& sorted, int begin_at)
{
    std::vector<int> reversed(qualities.size());
    sorted.resize(qualities.size());
    auto begin = reversed.begin();
    begin += begin_at;
    std::iota(begin, reversed.end(), 0);
    std::sort(begin, reversed.end(), [&](int a, int b) {return qualities[a+begin_at] > qualities[b+begin_at];});
    for(int i=begin_at; i<(int)sorted.size(); i++)
        sorted[reversed[i]] = i;
}
