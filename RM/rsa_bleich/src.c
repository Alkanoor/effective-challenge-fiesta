...
void puts_and_flush(const char *string) {
  puts(string);
  fflush(stdout);
}
...
int decrypt_buffer(RSA *rsa, const BUFFER_t *b_cipher, BUFFER_t *b_plain) {
  int size;

  buffer_init(b_plain, RSA_size(rsa));

  size = RSA_private_decrypt(b_cipher->size, b_cipher->buf, b_plain->buf, rsa, RSA_PKCS1_PADDING);
...
int encrypt_buffer(RSA *rsa, const BUFFER_t *b_plain, BUFFER_t *b_cipher) {
  buffer_init(b_cipher, RSA_size(rsa));

  if(RSA_public_encrypt(b_plain->size, b_plain->buf, b_cipher->buf, rsa, RSA_PKCS1_PADDING) < 0) {
